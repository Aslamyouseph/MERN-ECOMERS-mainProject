...
function BusinessLaptopUpdate() {
    ...
    const fetchLaptopDetails = async () => {
      try {
        const res = await fetch(
          `${process.env.REACT_APP_API_BASE_URL}/api/admin/getBusinessLaptopDetails/${id}`,
          {
            method: "GET",
            credentials: "include",
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/admin/updateBusinessLaptop/${id}`,
        {
          method: "PUT",
          credentials: "include",
          body: formDataToSend,
  ...
  return (
    <div className="laptop-form-container min-h-screen flex items-center justify-center py-8 bg-gradient-to-r from-purple-400 via-pink-500 to-red-500">
      <div className="laptop-form bg-white shadow-xl rounded-lg w-3/4 sm:w-1/2 md:w-1/3 p-8">
        <h2 className="laptopName">Update Business Laptop</h2>
...
export default BusinessLaptopUpdate;
